<?php
    ...
    $logout=new connect;
    $update_partner=new connect;
    $update_partner_show=new connect;
    if(isset($_GET['update_partner'])){
        $id=$_GET['update_partner'];
        $_SESSION['insert']=$_GET['update_partner'];
        $updatep=$update_partner_show->update_partner_show($id);
    }
    if(isset($_POST['regis'])){
        ...
        $update_partner->update_partner($id,$name,$lastname,$user,$password);
        header('location:show_partner.php');
    }
...
<?php $fuser=mysqli_fetch_array($updatep)?>
