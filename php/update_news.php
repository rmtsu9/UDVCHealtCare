<?php
    ...
    $logout=new connect;
    $in_news=new connect;
    $update_news=new connect;
    $update_news_show=new connect;
    date_default_timezone_set("America/New_York");
    if(isset($_GET['update_news'])){
        $id=$_GET['update_news'];
        $_SESSION['insert']=$_GET['update_news'];
        $updaten=$update_news_show->update_news_show($id);
    }
    if(isset($_POST['regis'])){
        $id=$_SESSION['insert'];
        $news=$_POST['news'];
        $date=$_POST['date'];
        $update_news->update_news($id,$news,$date);
...
<?php $fuser=mysqli_fetch_array($updaten)?>
<label for="news"><p>news</p></label>
    <input type="text" name="news" required value="<?php echo$fuser['news'] ?>">
    <label for="date"><p>date</p></label>
    <input type="text" name="date" required value="<?php echo$fuser['date'] ?>">
